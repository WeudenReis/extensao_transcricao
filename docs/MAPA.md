...
| `server/src/routes/painelInterno.ts` (395) | Consultas ao painel interno que a EXTENSÃO precisa fazer: GET /api/painel/vendedores        → seletor de vendedor (apresentação agendada) GET /api/painel/onboar | PainelInternoRouterDeps, createPainelInternoRouter |
...
| Arquivo | O que resolve | Exporta |
|---|---|---|
| `extension/content/aba-reuniao.js` (587) | A aba "Reunião" — o painel do Copiloto, feito do mesmo material. | __cpmAba |
...
| `extension/content/fluxo-reuniao.js` (3028) | O fluxo de marcar reunião, passo a passo, dentro da aba lateral. | __cpmFluxo |

## extension/background

| Arquivo | O que resolve | Exporta |
|---|---|---|
| `extension/background/service-worker.js` (348) | Service worker da extensão — a ponte entre o botão no chatPro e o backend. | — |

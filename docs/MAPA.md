...
|---|---|---|
| `server/src/chatpro/client.ts` (486) | Entrega da transcrição ao **chatPro Chat** (produto de conversas — não confundir com a "chatPro API", que é a de instâncias/WhatsApp). | MAX_TENTATIVAS, MAX_CARACTERES_COMENTARIO, backoffMs, ChatproPayload, ResultadoEntrega, PROVIDERS +7 |
...
| `server/src/config.ts` (395) | Carrega e valida as variáveis de ambiente com zod. | ehHostLocal, painelUrlProtegeOToken, Config, ConfigError, faltamNoChatpro, recallConfigWarnings +1 |
| `server/src/crypto.ts` (56) | Cifra segredos (refresh token do Google) com AES-256-GCM. | encryptSecret, decryptSecret |
| `server/src/db.ts` (1611) | Persistência em SQLite (better-sqlite3) com migrations inline. | LinkRow, TranscriptSentRow, GoogleTokensRow, SubscriptionRow, VoreoQueueRow, CaptureStatus +15 |
...
| `server/src/index.ts` (326) | Bootstrap: Express + rotas + tarefas de startup (diretório do DB garantido no construtor do Db; renovação de subscription no boot, após o OAuth conectar e a cad | — |
...
| `server/src/recall/criarReuniao.ts` (358) | Colocar o bot numa reunião — o caminho ÚNICO. | JANELA_DEDUP_MS, JANELA_SEM_BOT_MS, ANTECEDENCIA_MINIMA_JOIN_AT_MS, TOLERANCIA_MESMO_HORARIO_MS, CriarReuniaoEntrada, ResultadoCriacao +3 |
...
| `server/src/stt/decode.ts` (153) | Decodifica um arquivo de áudio (webm/opus) para PCM mono 16 kHz float32, usando o ffmpeg baixado pelo pacote ffmpeg-static (sem instalação manual). | decodeToPcm16kMono, SAMPLE_RATE, decodeChunksToPcm, medirVolume, writeWav |
...
| `server/src/stt/local.ts` (156) | Transcrição 100% LOCAL e GRATUITA com Whisper via transformers.js. | cleanText, isHallucination, LocalWhisperProvider |
...
| Arquivo | O que resolve | Exporta |
|---|---|---|
| `extension/background/service-worker.js` (305) | Service worker da extensão — a ponte entre o botão no chatPro e o backend. | — |
